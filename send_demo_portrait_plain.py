...
CHAT_ID="6817196309"
OUT="/home/jibril/match_analyzer/cards/card_portrait_plain.png"
...
def generate_card(batch, title="Matchs en vue"):
    ...
    for idx, ev in enumerate(batch[:6], 1):
        ...
        d.text((x1-12-cw,y+12), comp, fill=accent, font=fB)
        score=f"{ev.ph}-{ev.pa}"; sw=d.textbbox((0,0),score,font=fT)[2]
        d.text((x0+(x1-x0-sw)/2, y+70), score, fill=accent, font=fT)
        d.text((x0+16, y+130), f"{ev.home:<12} G% {ev.hg}", fill=text, font=fB)
        d.text((x0+300, y+130), f"{ev.away:<12} G% {ev.ag}", fill=text, font=fB)
        y+=block_h+12
...
def main():
    ...
    path=generate_card(batch,"Matchs en vue — fond uni")
    with open(path,'rb') as f:
        r=requests.post(f"https://api.telegram.org/bot{TOKEN}/sendPhoto",
            files={'photo':f},
            data={'chat_id':CHAT_ID,'caption':'⚽ GABFOOT — fond uni, texte clair (portrait)'} )
